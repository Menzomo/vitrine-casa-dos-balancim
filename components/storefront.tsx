...
import { Product, Category, getBrands } from '@/lib/products'
...
export function ProductCard({ product }: { product: Product }) {
  return <article className="group flex h-full flex-col overflow-hidden rounded border border-[#E7E7E5] bg-white transition duration-300 hover:-translate-y-1 hover:border-[#D4B05A] hover:shadow-lg hover:shadow-[#B58A2E]/10">
    <Link href={product.permalink} className="relative block aspect-square overflow-hidden bg-[#FAF8F3]" aria-label={`Ver ${product.title}`}>
...
export function FilterPanel({ selectedBrand, selectedCategory, inStock, onBrand, onCategory, onStock }: { selectedBrand: string; selectedCategory: string; inStock: boolean; onBrand: (v: string) => void; onCategory: (v: string) => void; onStock: (v: boolean) => void }) {
